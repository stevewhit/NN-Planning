...
BEGIN
	...
	/**************************************************************************************************************
		...
		---------
		Note: Declare this above the if/else blocks so that we can return an empty dataset if the @startDate 
			  and @endDate are invalid.
	***************************************************************************************************************/
	...
	-- Verify @startDate is a valid date in the quotes table AND that there are at least 5 additional quotes after 
	-- the @endDate otherwise the futureFiveDayPerformance SP will not perform the calculations correctly.
	IF (SELECT COUNT(*) FROM StockMarketData.dbo.Quotes WHERE [CompanyId] = @companyId AND [Date] <= @startDate) > 0 AND 
	   (SELECT COUNT(*) FROM StockMarketData.dbo.Quotes WHERE [CompanyId] = @companyId AND [Date] > @endDate) >= 5
	BEGIN
		...
		****************************/
		...
		EXECUTE GetFutureFiveDayPerformance @companyId, @startDate, @endDate, @riseMultiplier, @fallMultiplier
	...
	ELSE
	BEGIN
		PRINT 'Cannot create training dataset for company (' + convert(nvarchar, @companyId) + ') because the Quotes table is missing quotes between the @startDate and the @endDate.'
	END
